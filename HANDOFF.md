...
**Git:** branch `main`, repo público em https://github.com/SaLuanVitor/rag-auditado-ptbr.
**Este bloco não afirma contagem nem hash de propósito** — um cabeçalho que afirma o próprio estado
de git fica falso no instante em que é commitado, e "afirmação escrita quando era verdadeira e nunca
revisitada" é a classe de defeito dominante deste projeto. Confira você:
`git log --oneline | awk 'END{print NR}'` e `git status -sb`
...
| d   | O conteúdo de `rlm/rag-prompt` | **Fechada hoje**, puxando do hub: **autoriza a abstenção** e impõe **teto de três frases**, que ninguém tinha visto. Transcrito na AULA-21, com data, porque o valor pode mudar sem o repositório mudar. E são **sete** arquivos que o puxam, não três: seis `.py` mais o `00-SimpleRAG/04_LangGraph_RAG.ipynb`, que não é cópia do `.py` de mesmo nome. Contar com `--include=*.py` devolve seis, e foi assim que o número entrou errado em cinco superfícies |
