...
- **API removida, e não só depreciada**: `retriever.get_relevant_documents(question)` (`02-LangChain-AdaptiveRAG.py:132`), no mesmo gênero do `llm(...)` da Aula 20. **Medido no `langchain-core` 0.3.33**, que o repositório pina: a chamada funciona e emite um `LangChainDeprecationWarning` dizendo que o método "was deprecated in langchain-core 0.1.46 and will be removed in 1.0". **Medido em 15/09/2026 no `langchain-core` 1.6.3**, corrente no PyPI: `BaseRetriever` não expõe mais o método, e a linha levanta `AttributeError`. O `invoke` que o aviso indicava continua funcionando. As duas medições valem ao mesmo tempo, cada uma sobre a sua versão, e é a distância entre elas que mede o quanto o repositório envelheceu: mesmo alvo de remoção que o `llm(...)`, depreciado trinta e nove versões de patch antes, e os dois cumpridos.
